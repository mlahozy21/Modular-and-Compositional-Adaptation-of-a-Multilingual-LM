...
the 24 layers --- 48 scalars, which does require gradient descent. The two-scalar
model is the special case $\lambda_L[\ell]=\lambda_L$, $\lambda_G[\ell]=\lambda_G$,
so the per-layer fit starts from it and departs only where depth pays: whatever
it gains is what letting $\lambda$ vary with depth is worth, and not an artefact
of where the optimiser began. The step budget is fixed in advance by a rule
rather than by a stopping criterion: $\min(150,\max(30,3N/16))$ steps, where $N$
is the number of blocks in the training split and $16$ the batch size in blocks,
so the expression is three passes over that split, floored and capped. Here
$N=366$ gives $68$ steps. Validation and test are read only once the budget has
run out. These per-layer $\lambda$, unlike the
...
\paragraph{The loss is register-specific.}
...
precisely the tokens that mark narrative prose. And one caveat is large
