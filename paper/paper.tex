...
implies a scale. The projection of $\lambda(\delta_L+\delta_G)$ onto $\delta_L$
is $\lambda\lVert\delta\rVert(1+c)$, so the $\lambda$ that keeps the shared part
from being counted twice --- the one the double-counting account calls for --- is
$1/(1+c)$, here $0.88$. The fitted optimum is $(0.3,0.3)$: over $c\in[0,1]$ the
