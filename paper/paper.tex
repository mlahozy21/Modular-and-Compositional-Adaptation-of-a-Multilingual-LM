...
nothing in the fit can have consulted them. The $\lambda$ are left unconstrained: since
$\tau_L$ hurts the target on its own, a layer that wants $\lambda_L<0$ is a
result, and bounding them to $[0,1]$ would have hidden it.
...
thin.} The composition's gain is sixteen times larger in the first eighth of a
block ($-0.068$ bpb) than in the last ($-0.004$): with little left context the
model falls back on its priors, and that is what the modules supply. The genre
module shows the same thing from the other side, nearly harmless at the start
($+0.010$) and steadily more damaging as context accumulates ($+0.040$), which
is what an over-scaled prior does once the evidence is in. On the frequency
...
Three things survive. First, composition on this cell is real but it is mostly a
statement about magnitude: two modules that individually damage the target,
averaged or scaled down, improve it, and one module at a fitted scale recovers
roughly seven tenths of what the pair does. Task arithmetic already tunes a
scaling coefficient \cite{ilharco2023}; what this cell adds is a measurement of
what the second module buys on top of it --- $-0.0041$ bpb, consistent in sign
in all five chapters. Second, direction is not where we could find any headroom:
three corrections built on the geometry of the updates, in two different spaces,
buy at most a tenth of what one scalar per module buys. Third, the confound of
Section~1 is not a caveat here but the dominant effect, visible four times over:
in both modules hurting their own target cell, in the language
module's damage concentrating on the punctuation and function words of dialogue,
in that damage almost vanishing on the vocabulary its own corpus knows, and in
each module displacing the target along its confound's axis rather than its
own.
