...
nothing, while one fitted scalar per module does the work, which suggests that
interference-resolution merging addresses a problem this regime does not have.
\end{aclabstract}
...
survives are measured on these corpora, not taken from model cards. We select
...
low-rank update $\Delta W = (\alpha/r)BA$ is learned, with $B$ initialised to
zero so that training starts exactly at the backbone. A module is thus fully
described by one displacement $\Delta W$ per matrix. We adapt all seven
projection matrices of every block (\tec{q}, \tec{k}, \tec{v}, \tec{o},
...
informative figure is 1.24\% of the 1.20B that sit in the transformer blocks:
...
between them and none from the target: 696{,}334 segments and 11{,}135{,}049
words of legal Swedish for $\tau_L$ (2{,}520 optimisation steps), and
2{,}483{,}074 segments and 48{,}601{,}840 words of literary non-Swedish for
$\tau_G$ (9{,}810 steps). Both learn what they were given
...
For each adapted matrix we take orthonormal bases of the two modules' column
spaces --- the output directions each one writes into --- and compute the
singular values of $U_L^{\top}U_G$, the cosines of the principal angles between
them. The control matters here: two \emph{random} rank-$r$ subspaces of the same
...
\cite{wortsman2022,ilharco2023}. Stacking instead of adding, as in MAD-X \cite{pfeiffer2020}, is not
open to us: it trains the second module on top of the first, and the task
requires them independent. The standard response within weight space is to tune
...
already writes:
\begin{equation}
\theta_0 \;+\; \Delta W_L \;+\; \bigl(I - U_L U_L^{\top}\bigr)\,\Delta W_G .
\end{equation}
It has no free parameters, since $U_L$ comes from weights we already have; it is
self-targeting, because where the subspaces are orthogonal the projection is
nearly the identity, so we never choose which matrices to apply it to; and it
stays in the family, since $(I-U_LU_L^{\top})\Delta W_G = s(P^{\perp}B_G)A_G$ is
...
\emph{whole} training split (366 blocks, 753 kB). Validation and test are never
consulted for the choice.

We also fit a per-layer variant, $\lambda_L[\ell],\lambda_G[\ell]$ for each of
the 24 blocks --- 48 scalars, which does require gradient descent. It is
...
independent would inflate the sample from five chapters to 126 blocks; it
narrows the intervals by a factor of about $1.6$, on an independence that does
not exist. We report only the chapter-level intervals.

\emph{Five clusters bound what any interval can say.} Resampling five chapters
with replacement admits only $\binom{9}{5}=126$ distinct outcomes --- a
coincidence of arithmetic with the block count above, and unrelated to it --- so
the percentile interval is, to a good approximation, the range of the five
...
proj $\tau_L+P^{\perp}\tau_G$ & 1.1411 & $+0.0366$ & 25.21 & $[+0.031,+0.039]$ \\
proj $\tau_G+P^{\perp}\tau_L$ & 1.1415 & $+0.0370$ & 25.24 & $[+0.032,+0.040]$ \\
\bottomrule
\end{tabular}
...
is worth once this one has been scaled. The two dashes are by design --- the
uniform average fits nothing, and the per-layer variant spends a step budget
fixed in advance --- so neither consults a training score.}
\label{tab:sup}
\end{table}
...
may claim, and it is not the one usually made. Against the \emph{unscaled} genre
module the composition wins by $-0.039$ --- but a genre module scaled the same
...
range of those five numbers. The attribution depends on the order one decomposes
in and we report both: crediting genre first leaves the language module worth
26\% of the total, crediting language first leaves genre worth 67\%. The two do
not add to one because the modules are sub-additive --- worth $0.0172$
separately against $0.0159$ together --- so neither ordering alone is the
answer. Averaging each module's marginal contribution over both orderings, which
for two players is the Shapley value, gives \textbf{30\% to the language module
and 70\% to the genre one}. These are shares between two \emph{mixtures}, not
between a clean language factor and a clean genre factor: by
Section~\ref{sec:interp} each module moves the target along its confound's axis,
so what the split divides is contaminated on both sides. Two scalars do not overfit: fitting them leaves the
...
\begin{axis}[
  ...
  font=\footnotesize,
  xlabel={transformer block}, ylabel={fitted $\lambda$},
...
terminal blocks --- adjacent to the embedding matrix and the output vocabulary
...
the first eighth --- 62\% of it is lost by the second --- while the genre module
on its own grows steadily more damaging as context accumulates. Negative is
better than $\theta_0$.}
\label{fig:posicion}
\end{figure}
...
fall is a cliff rather than a slope --- most of the gain is gone by the second
...
independent pushes. Over 2{,}048 vectors per layer the mean cosine between
$\delta_L$ and $\delta_G$ is $+0.132$, against a scale of $1/\sqrt{d}=0.022$ for
a single random cosine --- six times larger, and far outside what sampling noise
could produce. So the two modules push the activations in substantially the same
direction while writing into near-orthogonal weight coordinates. But a cosine of
$0.132$ predicts a per-module scale of $1/(1+c)=0.88$, and the fitted optimum is
$(0.3,0.3)$: over $c\in[0,1]$ the predicted scale never falls below one half,
and reaching three tenths would need $c=2.33$. Whatever forces
$\lambda$ down is not the double-counting the geometric account posits.

\paragraph{The confound is a measurable direction --- and it is the only thing
the axes see.} We build two axes from backbone centroids with a single factor
varying: genre as \emph{sv literary} $-$ \emph{sv legal} (positive $=$ more
literary), language as \emph{non-sv literary} $-$ \emph{sv literary} (positive
$=$ less Swedish). Decomposing each module's displacement in that plane,
...
and at most 7.4\%, so there is almost nothing to remove. A sharper probe would
raise that fraction; what this establishes is that the correction available
within the plane we could build is nil, not that no directional correction
exists. For modules trained on factors this disjoint, the interference that TIES
\cite{yadav2023} and DARE \cite{yu2024} resolve appears not to be the binding
constraint; total displacement is.

\paragraph{Direction decides the sign, magnitude decides how much.} That
conclusion needs a bound, or it reads as though any direction would serve. We
built an adapter with the same rank and the same per-matrix Frobenius norm as
the fitted composition but with $B$ and $A$ drawn at random, over three seeds.
It costs $+0.0071$ bpb where the composition gains $-0.0157$ --- a gap of
$0.0228$ --- and the three draws fall within $0.0009$ of one another. The random
adapter also displaces the activations by half as much, $\lVert\delta\rVert /
\lVert h\rVert$ of $0.069$ against $0.124$, since it acts on input directions
the model rarely visits. The control is therefore conservative twice over:
arbitrary directions fail to help while moving the model less, and matching
their displacement where it counts could only make them worse. Moving further
helps when the direction is one the modules learned and hurts when it is not, so
what the scale governs is how large a gain, not whether there is one.

\section{Discussion and limitations}

Two things survive. Task arithmetic already tunes a scaling coefficient
\cite{ilharco2023}; what this cell adds is a measurement of what a second module
buys once the first is already scaled --- $-0.0041$ bpb, consistent in sign in
all five chapters --- and of how little is left to gain from the geometry once
it is. And the confound of Section~1 is not a caveat here but the dominant
effect: both modules hurt the very cell they were built to serve, the language
module's damage lands on the punctuation and function words of dialogue, and
each module displaces the target along its confound's axis rather than its own.

\paragraph{Limitations.} \emph{Everything here is translated text}: DGT is a
...
``this factor carries less'' from ``this module saw less text''. \emph{No norm-matched control}: we claim total
displacement governs the composition but never measured it, in state space, for
the scaled single-module systems --- one forward pass. \emph{No
...
\bibitem{pfeiffer2020} J. Pfeiffer, I. Vuli\'c, I. Gurevych, S. Ruder. 2020.
...
\bibitem{yadav2023} P. Yadav, D. Tam, L. Choshen, et al. 2023. TIES-Merging:
Resolving Interference When Merging Models. \emph{NeurIPS}.
\bibitem{yu2024} L. Yu, B. Yu, H. Yu, et al. 2024. Language Models are
